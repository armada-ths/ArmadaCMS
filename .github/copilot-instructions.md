...
- **Feature flags**: `FeatureFlagController` auto-updates `EXHIBITOR_SIGNUP` based on IR/FR date windows on every GET. `models/feature_flag.go` has `DefaultFeatureFlags` that seed the DB on startup.
