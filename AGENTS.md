...
- Validate backend changes with `go test -race -count=1 ./...`; validate admin UI changes in `frontend/` with `npm run lint:check`, `npm run type-check`, and `npm run format:check`.
